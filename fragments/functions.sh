...
displaynotification() { # $1: message $2: title
    ...
    if [[ "$($swiftdialog --version | cut -d "." -f1)" -ge 2 && "$NOTIFY_DIALOG" -eq 1 ]]; then
        # if DIALOG_KEY is provided, add it to the arguments for Swift Dialog, otherwise call Swift Dialog without authorization key
        if [[ $DIALOG_KEY != "" ]]; then
            "$swiftdialog" --key "${DIALOG_KEY}" --notification --title "$title" --message "$message"
        else
            "$swiftdialog" --notification --title "$title" --message "$message"
        fi
        # log authorization error, exit code 30
        if [[ $? -eq 30 ]]; then
            printlog "Swift Dialog not authorized to run" WARN
        fi
